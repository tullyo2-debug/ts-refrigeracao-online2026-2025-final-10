...
async function ordens(c) {
  ...
  const render=f=>{const list=(data||[]).filter(x=>`${x.numero} ${x.clientes?.nome||''} ${x.status||''}`.toLowerCase().includes((f||'').toLowerCase()));document.querySelector('#ostable').innerHTML=table(list.map(x=>`<tr><td><b>#${x.numero||''}</b></td><td>${esc(x.clientes?.nome||'')}</td><td>${esc(x.equipamentos?`${x.equipamentos.marca||''} ${x.equipamentos.modelo||''}`.trim():'')}</td><td>${esc(x.agendamentos?.data_agendamento||'')}</td><td><span class="badge">${esc(x.status||'')}</span></td><td>${money(x.valor_total)}</td><td><button class="link" data-os="${x.id}">Abrir / Editar</button><button class="danger-link" data-os-del="${x.id}">Excluir</button></td></tr>`),['OS','Cliente','Equipamento','Agendamento','Status','Total','Ações']);document.querySelectorAll('[data-os]').forEach(b=>b.onclick=()=>osForm(list.find(x=>x.id===b.dataset.os)));document.querySelectorAll('[data-os-del]').forEach(b=>b.onclick=async()=>{if(!confirm('Excluir esta OS definitivamente? Todos os itens, fotos, histórico, assinatura e recebimentos vinculados serão excluídos.'))return;const {error}=await supabase.from('ordens_servico').delete().eq('id',b.dataset.osDel);if(error)return alert(`Não foi possível excluir a OS: ${error.message}`);loadPage('os')})}
...
async function osForm(x={}) {
  ...
  <label>Status financeiro${formSelect('osPayStatus',pagamentosStatus,x.pagamento_status||'Pendente')}</label><label>Forma de pagamento<input class="compact-field" id="osForma" value="${esc(x.forma_pagamento||'')}"></label>
  <label class="wide compact-area">Problema relatado<textarea id="osProblem" rows="3">${esc(x.problema_relatado||'')}</textarea></label><label class="wide compact-area">Diagnóstico<textarea id="osDiag" rows="3">${esc(x.diagnostico||'')}</textarea></label><label class="wide compact-area">Serviço executado<textarea id="osExec" rows="3">${esc(x.servico_executado||'')}</textarea></label><label class="wide compact-area">Observações<textarea id="osObs" rows="3">${esc(x.observacoes||'')}</textarea></label>
  </div>
  ...
  <div class="actions"><button class="secondary" id="osBack">Cancelar</button>${x.id&&x.status!=='Concluída'&&x.status!=='Cancelada'?'<button class="secondary" id="closeOS">Finalizar serviço</button>':''}${x.id?'<button class="danger" id="deleteOS">Excluir OS</button>':''}<button class="primary" id="saveOS">${x.id?'Salvar alterações':'Criar OS'}</button></div></div>`
  ...
  const saveOSRecord=async()=>{const {data:{user}}=await supabase.auth.getUser();const payload=collect();payload.criado_por=x.criado_por||user?.id;if(!payload.cliente_id)throw new Error('Selecione o cliente.');if(payload.status==='Concluída'&&!x.data_conclusao)payload.data_conclusao=new Date().toISOString();let q=x.id?supabase.from('ordens_servico').update(payload).eq('id',x.id).select('id,numero,valor_total,status').single():supabase.from('ordens_servico').insert(payload).select('id,numero,valor_total,status').single();const {data,error}=await q;if(error)throw error;await saveChildren(data.id);return data}
  document.querySelector('#saveOS').onclick=async()=>{try{const data=await saveOSRecord();alert(`OS #${data.numero} salva com cliente, equipamento, agendamento, serviços e peças vinculados.`);loadPage('os')}catch(e){alert(`Não foi possível salvar a OS: ${e.message}`)}}
  document.querySelector('#deleteOS')?.addEventListener('click',async()=>{if(!confirm(`Excluir a OS #${x.numero} definitivamente? Todos os dados vinculados, inclusive recebimentos, serão excluídos.`))return;const {error}=await supabase.from('ordens_servico').delete().eq('id',x.id);if(error)return alert(`Não foi possível excluir a OS: ${error.message}`);alert('OS excluída com sucesso.');loadPage('os')})
  document.querySelector('#closeOS')?.addEventListener('click',async()=>{if(!confirm('Finalizar o serviço e fechar esta OS? Depois da finalização você poderá registrar o recebimento imediatamente.'))return;try{document.querySelector('#osStatus').value='Concluída';const data=await saveOSRecord();if(confirm(`OS #${data.numero} finalizada. Deseja registrar o recebimento agora?`)){const formaPagamento=document.querySelector('#osForma')?.value||'';await loadPage('financeiro');paymentForm({os_id:data.id,valor:Number(data.valor_total||0),forma_pagamento:formaPagamento})}else{loadPage('os')}}catch(e){alert(`Não foi possível finalizar a OS: ${e.message}`)}})
}

async function syncPaymentStatus(osId){if(!osId)return;const [{data:os},{data:payments}]=await Promise.all([supabase.from('ordens_servico').select('valor_total').eq('id',osId).maybeSingle(),supabase.from('pagamentos').select('valor').eq('os_id',osId)]);if(!os)return;const total=Number(os.valor_total||0),pago=(payments||[]).reduce((sum,r)=>sum+Number(r.valor||0),0);const status=pago<=0?'Pendente':pago+0.005<total?'Parcial':'Pago';const {error}=await supabase.from('ordens_servico').update({pagamento_status:status}).eq('id',osId);if(error)throw error}
...
async function financeiro(c) {
  ...
  document.querySelector('#payTable').innerHTML=table((payments||[]).map(x=>`<tr><td>${esc(x.ordens_servico?.numero||'')}</td><td>${esc(x.ordens_servico?.clientes?.nome||'')}</td><td>${esc(x.forma_pagamento||'')}</td><td>${money(x.valor)}</td><td>${esc(isoDate(x.data_pagamento))}</td><td><button class="link" data-pay="${x.id}">Editar</button><button class="danger-link" data-pay-del="${x.id}">Excluir</button></td></tr>`),['OS','Cliente','Forma','Valor','Data','Ações']);
